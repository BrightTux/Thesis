...
\includegraphics[width=.8\textwidth]{image/retrievalTwo/VISERinterface2.png}
...
The \textit{Precision@K} metric was used to identify the number of retrieved results which are relevant to the user while the \textit{nDCG} metrics is used to measure how well the retrieval engine display the results to the users. 

In order to evaluate the performance of the proposed method, the evaluation of both the trajectory and color terms were separated. A group of volunteers were introduced to the retrieval engine along with it's features and a sample use case was demonstrated, then, they were given full control to draw any query they were interested in. Next, the volunteers were tasked to provide feedback by rating the results with a scale of 1 to 5 with 5 being a result that closely resembles the input query. 

The volunteers were given full control of the input query \mathbb{P} as it closely resembles a real world scenario. With that in consideration, the ground truth annotation for each of those queries during the 30 days data is not available, hence, the recall rate can not be measured in this retrieval engine. 

While the \textit{Precision@K} metrics is a commonly used metric for retrieval system, the main disadvantage of this metric is that the evaluation done in a coarse manner as it only provides a binary evaluation of whether or not a particular result is relevant to the query. On the other hand, the \textit{DCG} metric is able to provide a more granular evaluation of the proposed method as the results will also be evaluated based on its ordered rank along with the relevancy of the result. Figure \ref{fig:dcgGain} illustrates the decreasing gain value of the result as the ranking increases. 

\cc{work from here onwards}

\begin{figure}[h]
\centering
% Use the relevant command to insert your figure file.
% For example, with the graphicx package use
  \includegraphics[width=0.9\textwidth]{image/Discount_vs__Rank.png}
...
Since the Discounted Cumulative Gain metric produces an unbounded value ([0, $\infty$) range), it is not suitable for use when averaging across multiple independent queries. Hence, the normalized Discounted Cumulative Gain (nDCG) metric is used to convert the unbounded value into a [0, 1] range.


\begin{equation}
\label{eq:nDCG}
\it{n}DCG_P = \frac{DCG_P}{IDCG_P} \textnormal{, where}
\end{equation}


\begin{equation}
\label{eq:IDCG}
IDCG_P \textnormal{ is the ideal DCG score at Rank P in the ideal ranking order}
\end{equation}
...
\subsubsection{Vehicle Trajectory}
 The volunteers were asked to draw at least 1 trajectory as an input query, next, they were asked to rate each of the resulting trajectory videos with a scale from 1 to 5: where 1 signifies the least relevant, and 5 being the most relevant. Each input query would return at least 25 results which are ordered randomly, this prevented the volunteers from having a preconceived idea of how the results should be ordered. This helped by removing biasness when the volunteers were tasked with rating each results. 

\subsubsection{Vehicle Color \& Color Mode}
A list with a total of 330 video snippets (30 video snippets for each of the 11 common colors) were presented to the volunteers. These 30 video snippets from each colors were selected using the top 30 results in the database which contained the highest probability for each of the colors. Then, the volunteers were asked to determine the relevance of the color term to that particular vehicle for each of the 330 video snippets with the same scale from 1 to 5. Since there were 4 types of colors modes (LUV, HSV, Lab, Average), the volunteers were split into 4 groups to evaluate each color mode.  
