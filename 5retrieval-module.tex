...
Instead of treating each spatio-temporal atom cubes as an unique document, in this framework, the unique identifiers ($atom_x$, $atom_y$, $atom_t$) of each atom will be used for the retrieval process. As mentioned above, the focus of this framework is veered towards the use of distance measures (see Section \ref{section:distancemeasures}) in order to retrieve results that are relevant.

In the same manner, the input query, $\mathbb{Q}$, also takes on the properties of the atom cubes' unique identifiers with the exception of the $atom_t$. As a substitute of the $atom_t$ identifier, the order of the input, $o_{i}$, is taken into consideration as it is used to mimic the motion of the target vehicle shot. However, to do so, first, the graphical query input needs to be converted into a set, $\mathbb{Q}$ by removing duplicate identifiers which was introduced by the mouse stroke. This will thus result with unique identifiers remaining in the set. 

Next, as the use of time and date semantics were extracted, these metadata can be used to filter and further narrow down the search scope.  
...
Along with that, in order to maximize the difference between the input and the video shots available in the database, the length of the query ($query_{length}$) is measured against the length of the individual records ($record[i]_{length}$) in the database,  
