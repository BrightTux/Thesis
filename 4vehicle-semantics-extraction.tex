...
The usage of all eleven colour terms to describe an ADC is essential as some colours bears high similarity against the other. A classic example would be the similarity between pink and red colour; while they are essentially two different colours belonging to a relatively similar hue family. Hence, the similarity score between both these red and pink colour should be rather high. This setup allows colours which are visually similar to be ranked higher when the video shots are retrieved using the retrieval engine.
...
Based on the results observed in Table \ref{tab:luvExample} and Table \ref{tab:hsvExample}, the similarity score obtained using Euclidean Distance in HSV colour space shows poorer performance than the score obtained using Riemersma's method. The similarity score obtained using the HSV colour space shows minimal variation between the colour terms. This inevitably makes it hard to determine the colour terms which best matches the ADC.
On the other hand, the results obtained using Riemersma's method shows clear distinction between contrasting colour terms which makes its easier for the determination of ADC term.
