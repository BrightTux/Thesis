...
With that in place, each of the blobs which were identified as a vehicle were tracked. However, blobs which were not identified as a vehicle was left as a possible candidate. This is done in view of misidentification due to poor lighting conditions or error during the background subtraction process. For each of the vehicle blobs, the semantic extraction module can now proceed.  
...
\label{eq:file_timing}
	Time_{Current Frame}  = (Video Duration \times \frac{Current Frame}{Total Frames}) + Time_{filename}, 
\end{align}

where~$VideoDuration$ is 6 minutes (see Section \ref{section:dataset_used}) and ~$Time_{filename}$ is the time information extracted from the last six digits of filename (excluding the file extension). Upon extracting these information, simple formatting is done to ensure these information conform to the typical SQL database requirements.

%As described in Section \ref{section:dataset_used}, the camera was set to record video clips from 8:30am to 6:30pm in the evening with each clip being 6 minutes long. 
...
The step to differentiate between chromatic and achromatic vehicles was implemented to overcome the shortcomings of the 960 color bins proposed for the Color Term Extraction process. Figure \ref{fig:hsvAllocated} illustrates the color bins generated, the details on how these color bins were allocated is discussed further in the following paragraphs. One of the main drawback of the proposed method  was the number of bins which represented the achromatic colors. From the figure, it is clear that while there are a substantial amount of bins (approximately 120 bins) representing the almost-pure-black darker tonnes, however, approximately only 15-30 bins were used to represent the lighter shades of achromatic colors. This imbalance, compounded with the noise captured (ie: road, other vehicles, trees) during the dominant color extraction process made it difficult for vehicles to be match with "White" color term. Hence, this step was introduced as a measure to counterbalance the weakness of the proposed method. 


This process of differentiating chromatic against achromatic vehicles starts off by comparing the cropped image, $I_{ori}$, against a grayscale version of the same image, $I_{grayscale}$. The conversion from RGB to grayscale is performed using $I_{grayscale_{(x,y)}} = 0.299 \cdot I_{ori}R_{(x,y)}+0.587 \cdot I_{ori}G_{(x,y)}+0.114 \cdot I_{ori}B_{(x,y)}$. However, as the grayscale image and RGB image does not have the same number of channels, the grayscale image is converted back to a RGB image prior to the comparison.
Next, the obtained absolute difference, $I_{Abs_{diff}} = \mid I_{ori} - I_{grayscale(RGB)} \mid$, between these images were subjected to a threshold process for all three channels in the RGB color space. 
The step was done to effectively amplify the difference between the original image and the grayscale image, pixels with values above the threshold value were set to the maximum value of 255. \\ 
Given that $V_{(x,y)}$ is the value of a pixel at location $(x,y)$, the threshold value $T_{Abs_{diff}}(x,y)$ can be obtained using:   

\begin{align*}
\label{eq:threshabsolutediff}
T_{Abs_{diff}}(x,y) = \alpha \cdot (255) + \bar{\alpha} \cdot(0) \\
\text{where} 
\begin{cases}
V_{(x,y)} \geq 35, & \alpha = 1\\
otherwise, & \alpha = 0
\end{cases}
\end{align*}
 ...
 Finally, the output of the process was converted to the grayscale color space. The hint of significant values at this stage provided an indication of substantial presence of chromatic hues for this particular vehicle.     

%Next, the obtained absolute difference between these images were subjected to a threshold process for all three channels in the RGB color space. The step was done to effectively amplify the difference between the original image and the grayscale image, pixels with values above the threshold value were set to the maximum value of 255. The threshold value used for this step was empirically set at 35. 
...
The aim of the \versionTwoExt is to provide alternative solutions and improve the performance of the \versionOneExt. In the following subsections, the color semantic extraction and motion semantic extraction is discussed in detail.
