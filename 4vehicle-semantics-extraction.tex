...
\textbf{1}   & Date                    & Filename extraction                                                                                                         \\ \hline
\textbf{2}   & Time                    & Filename extraction                                                                                                         \\ \hline
...
However, a vehicle's motion in a carpark may vary significantly from another vehicle's motion, instead of generating a fine representation of the vehicle's motion, the individual motions of the vehicle is captured. 


As the trajectory information from each vehicle is important, it is a paramount task to ensure that the motion information from each vehicle was captured and stored. Since the types of motion in a carpark scenario varies from each carpark user, instead of generating a fine representation of motion such the conventional motion vector, these information were stored in the form of four cardinal directional and four intermediate directional bins along with one which denotes minuscule and negligible motions. The directional bins are represented in Figure \ref{fig:cardinalbins}.

\cc{neeeds to re-write this portion to fill up with motion semantic info}
...
In this work, the motion of a vehicle is quantized into 8 directional bins along with 1 bin which denotes little to no significant movement. Figure \ref{fig:cardinalbins} illustrates this property, this setup allows complex motion information to be captured.
...
\cc{write more to fill up with motion semantic extraction}
