...
In order to further leverage on this concept, the quantization technique
...
To further accentuate on the concept of distance measure in this work, a simple
...
\end{tabular}}
\caption{List of Several Distance Measures and Their Pros and Cons}
...
As an example, the RGB colour space is an additive colour model, each of the
...
tuples. \citeA{de1971color} once said that "The explanation of the nature and
...
In a classic study of worldwide colour naming, \citeA{berlinandkay} proposed
...
However, \citeA{yu2018beyond} also remarked that most existing work on colour
...
leveraged upon. Recent survey done by \citeA{tian2017hierarchical} and
\citeA{chandran2017review} were studied to understand the general overview of
...
important applications. \citeA{hsieh2015vehicle}, \citeA{chen2014vehicle} and
...
colours drastically. As said by \citeA{albers2006interaction}, 'colour
...
\citeA{hsieh2015vehicle}'s work, the vehicles are grouped into seven categories
...
\begin{figure}[hbt!]
 ...
 (b) Chromatic Vehicles. Reproduced from \citeA{hsieh2015vehicle}.}
...
Along with that, occasionally the vehicle windows appear white due to the
...
vehicles were reported to be approximately 88\%. \citeA{chen2014vehicle} also
...
classification task, \citeA{kim2008deciding} aimed to understand the correlation
...
\citeA{zhang2017vehicle} and \citeA{hu2015vehicle} approached this challenge of
...
classification task. \citeA{zhang2017vehicle} mentioned that the
...
According to \citeA{hu2015vehicle}, there are two reason of choosing SVM over
...
the first convolutional layer produced in \citeA{hu2015vehicle}'s work. The
...
\citeA{liu2016deep}, \citeA{liu2016deep2} and \citeA{shen2017learning}) made use
of extracted colour features to assist with the re-identification process.
\citeA{liu2016deep} adopted the Probabilistic Latent Semantic Analysis (PLSA)
introduced by \citeA{van2009learning} to extract the colour features. A PLSA
...
\citeA{kim2008deciding}, the images in this work were segmented and represented
using L*a*b colour histogram of $10 \times 20 \times 20$ grids.
\citeA{liu2016deep2} applied deep learning techniques inspired by a triplet loss
network proposed by \citeA{ding2015deep}, the network used a new loss function
to accelerate the training convergence. The colour features were extracted using
the proposed network. Similarly, \citeA{shen2017learning} applied a deep
...
\citeA{van2009learning} proposed several variants of Probabilistic Latent
...
under a known white light source. Similar to \citeA{van2009learning}'s work,
\citeA{yu2018beyond} also applied PLSA to estimate the probability of colour
...
The PLSA model allows multiple colour terms to be named for each image. The
...
The work of \cite{zaslavsky2018efficient} took on another approach towards
colour tuples mapping. The authors aimed to study efficient compression in
colour naming and its evolution. \citeA{zaslavsky2018efficient} claims that
...
\citeA{zaslavsky2018efficient}'s work. The goal of this work is to design a
...
BGS methods are often used as there is no need of prior training on a machine to
...
videos, \citeA{lessard2016countingapp} applied the works of
\citeA{saunier2006feature} to extract motion information using KLT tracker.
...
saved into the database. Likewise, \citeA{castanon2016retrieval} categorised
...
\citeA{d2015designing} represent trajectories using spatio temporal cubes (See
Figure \ref{fig:spatiocube}). Similarly, \citeA{lai2015video} represented
...
\ref{fig:cvbr} presents a classification of the various approaches used in CVBR
systems.
...
Several authors (\citeA{feris2012large}, \cite{momin2015vehicle}, and
...
keywords. In a review, \citeA{bhaumik2016hybrid} claims that the use of textual
...
another classical method applied by retrieval engines. \citeA{flickner1995qbic}
...
trajectories, \citeA{lai2015video} allowed end users to draw trajectories that
