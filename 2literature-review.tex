...
\begin{comment}
--> ways of extracting motion: optical flow+histogram bin, bgs+movement,  
\end{comment}

Vehicle motion is an important feature that can be used to describe activities in a car park surveillance scene. While there are not many research done within the scope of a car park surveillance setting, there has been many works in the ITS domain which extracts vehicle motions. In most cases where the research setup involves a static camera sensor, the extraction of these motions can be broadly divided into two groups: i) Background subtraction (BGS) methods combined with vehicle centroid position for motion estimation; and ii) Feature (SIFT, HOG, Haar, etc) extraction combined with Optical flow for motion estimation. 
\cc{--> However, there are also several other methods that for motion extraction such as histogram bins....  }

Several authors such as \citeA{wang2016research}, \citeA{luvizon2017video}, and  \citeA{apeltauer2015automatic} extracted vehicle motion information using the first approach where a BGS method was employed. Generally, a clean model of background information is first obtained. When a new subject is introduced to the scene in the current frame, the differences between the background information and the current frame is extracted to indicate changes in the scene. When the subjects are moving in the scene, the data obtained from the subtraction process would correspond to motion information. Figure \ref{fig:bgs2} illustrates the process involve in BGS. 
...
However, as described in Section \ref{subsec:scope}, the bounding box of the vehicles is assumed to be provided. Hence, the trajectory representation methods are discussed in detail instead of the trajectory extraction process. 
In the works of \citeA{momin2015vehicle}, the authors only split the motion of the vehicles into 4 directions (LEFT, RIGHT, UP, DOWN). Similarly, the works of \citeA{feris2012large} splits the motion of vehicles into 12 motion directions. This was done by building a motion direction histogram, where bins that receives the highest number votes were saved into the database. 

\citeA{lai2015video}, \citeA{castanon2016retrieval} --> represent trajectories in 3d space

\cite{lessard2016countingapp} --> count vehicles in 500 hours


\subsection{Object Semantics Retrieval}

According to the survey by \cite{chandran2017review}, object retrieval algorithms can be broadly divided into two categories: i) String Matching Algorithm, and ii) Sketch Matching Algorithm. String matching algorithms typically converts subjects into a set of known keywords and match them using its semantic meanings. Several authors (\citeA{feris2012large}, \cite{momin2015vehicle}, and \cite{yang2015semantic}) has implemented string matching algorithms in their work.

In the case of vehicle trajectories, the queries used corresponds to the extracted motion terms such as "Turning into junction A" or "Enter from entrance B". 


as their queries were performed on specific scenes with little variation of motions.    


However, in a real life application, this method of segmenting different motions into




On the other hand, sketch matching algorithm 
by \citeA{castanon2016retrieval} \citeA{lai2015video}
...
castanon2016retrieval
feris2012large
lessard2016countingapp
